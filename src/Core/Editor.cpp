...
namespace EditorSystem
{
    ...
    void DrawUIInspector(Entity e, Registry &reg, float xPos, float &currentY, float panelWidth, Engine *engine)
    {
        ...
        if (GuiValueBox({xPos + 65, currentY, 80, 24}, "X", &ox, -2000, 2000, (engine->activeControlId == 52)))
            engine->activeControlId = (engine->activeControlId == 52) ? 0 : 52;
        if (GuiValueBox({xPos + 155, currentY, 80, 24}, "Y", &oy, -2000, 2000, (engine->activeControlId == 53)))
        ...
        if (GuiValueBox({xPos + 65, currentY, 80, 24}, "W", &sw, 1, 2000, (engine->activeControlId == 54)))
            engine->activeControlId = (engine->activeControlId == 54) ? 0 : 54;
        if (GuiValueBox({xPos + 155, currentY, 80, 24}, "H", &sh, 1, 2000, (engine->activeControlId == 55)))
    ...
    void FloatBox(const char* label, float* value, Rectangle bounds, int controlId, float min, float max, Engine* engine)
    {
        ...
        // UNIQUE BUFFER: Use an array to prevent different boxes from sharing the same text
        static char buffers[100][64];
        char* textBuffer = buffers[controlId % 100];

        // 1. Label
        if (label) DrawText(label, (int)bounds.x - 75, (int)bounds.y + (int)(bounds.height / 2) - 6, 12, GRAY);
        ...
        if (isTyping)
        {
            ...
            // Cancel on Escape
            if (IsKeyPressed(KEY_ESCAPE)) engine->activeControlId = 0;
        }
    }
    ...
    void DrawInspector(Entity e, Registry &reg, int screenWidth, int screenHeight, Engine *engine)
    {
        ...
        if (reg.HasComponent(e, COMP_RIGIDPHYSICS)) {
            ...
            FloatBox("Mass", &rp.mass, { xPos + 80, currentY + 20, panelWidth - 95, ctrlH }, 11, 1, 1000, engine);
            FloatBox("Restitution", &rp.restitution, { xPos + 80, currentY + 50, panelWidth - 95, ctrlH }, 12, 0, 1, engine);
            FloatBox("Friction", &rp.friction, { xPos + 80, currentY + 80, panelWidth - 95, ctrlH }, 13, 0, 1, engine);
            FloatBox("Gravity Scale", &rp.gravityScale, { xPos + 80, currentY + 110, panelWidth - 95, ctrlH }, 14, 0, 10, engine);

            rp.isStatic = GuiCheckBox({ xPos + 15, currentY + 145, 20, 20 }, "Static", rp.isStatic);
            rp.affectedByGravity = GuiCheckBox({ xPos + 120, currentY + 145, 20, 20 }, "Gravity", rp.affectedByGravity);
        ...
        if (reg.HasComponent(e, COMP_CIRCLECOLLIDER)) {
            ...
            FloatBox("Radius", &cc.radius, { xPos + 80, currentY + 20, panelWidth - 95, ctrlH }, 15, 1, 1000, engine);
            FloatBox("Off X", &cc.offset.x, { xPos + 80, currentY + 50, panelWidth - 95, ctrlH }, 16, -1000, 1000, engine);
            FloatBox("Off Y", &cc.offset.y, { xPos + 80, currentY + 80, panelWidth - 95, ctrlH }, 17, -1000, 1000, engine);

            cc.debugDraw = GuiCheckBox({ xPos + 15, currentY + 115, 20, 20 }, "Debug Draw", cc.debugDraw);
    ...
    void DrawSpriteEditor(Entity e, Registry &reg, float xPos, float &currentY, float panelWidth, Engine *engine)
    {
        ...
        if (GuiButton({xPos + 15, footerY, (panelWidth - 30) / 2, 25}, s.flipX ? "Flipped X" : "Normal X"))
            s.flipX = !s.flipX;
